...
app.post("/api/payment", async (req, res) => {
  try {
    ...
    // ✅ DÜZELTME: PayTR hash string sonuna MERCHANT_SALT eklenmeli
    const hash_str =
      PAYTR_MERCHANT_ID +
      user_ip +
      merchant_oid +
      ...
      currency +
      test_mode +
      PAYTR_MERCHANT_SALT; // ← burası eksikti!
...
app.post("/api/paytr-notification", async (req, res) => {
  ...
  if (status === "success" && process.env.GMAIL_USER) {
    transporter.sendMail({
      ...
      html: `<h2 style="color:#16a34a">Ödeme Onaylandı!</h2>
        <p>Sipariş: <b>${merchant_oid}</b></p>
        <p>Tutar: <b>${parseInt(total_amount) / 100} ₺</b></p>`,
    }).catch(e => console.error("Mail err:", e));
  }
  res.send("OK");
});
